...
值类型的对象只有 (t T) 结构的方法，虽然值类型的对象也可以调用`` func (t *T) do{}`` 方法,是因为底层把T转化为指针类型
...
所有unsafe.Pointer 是指针和uintprt之间的桥梁, unsafe.Pointer 底层是个int类型,参见demo:
```go
dog := Dog{"little pig"}
dogP := &dog
...
语法：``t.(xxxx)``,判断你要访问的变量是否是预期的类型，常用示例

```go
    if value, ok := t.(helper);ok {
        // t 是helper类型时的义务处理
    }else {
        // t不是helper类型的处理
    }
